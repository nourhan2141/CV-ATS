...
from app.utils.pymupdf_rag import to_markdown
...
from app.utils.transform import transform_parsed_data
...
class PDFHandler:
    ...
    def extract_json_from_pdf(self, pdf_path: str) -> Tuple[Optional[JSONResume], Optional[Any]]:
        try:
            logger.debug(f"📄 Extracting text from PDF: {pdf_path}")
            
            # Run deterministic ATS formatting checks
            ats_report = None
            try:
                ats_checker = DeterministicATSChecker(pdf_path)
                ats_report = ats_checker.analyze()
                logger.debug("✅ Successfully ran deterministic ATS checks")
            except Exception as e:
                logger.warning(f"⚠️ Failed to extract ATS formatting report: {e}")

            text_content = self.extract_text_from_pdf(pdf_path)

            if not text_content:
                logger.error("❌ Failed to extract text from PDF")
                return None, ats_report
            ...
            return self._extract_all_sections_separately(text_content), ats_report
    ...
    def _extract_all_sections_separately(
        ...
        def extract_with_retry(section_name):
            result = self._extract_section_data(text_content, section_name)
            if not result:
                logger.warning(f"⚠️ First attempt failed for {section_name}. Retrying once...")
                result = self._extract_section_data(text_content, section_name)
            return result

        with concurrent.futures.ThreadPoolExecutor(max_workers=len(sections)) as executor:
            future_to_section = {
                executor.submit(extract_with_retry, section_name): section_name
            ...
            for future in concurrent.futures.as_completed(future_to_section):
                ...
                try:
                    ...
                    if section_data:
                        ...
                        logger.error(f"⚠️ Failed to extract {section_name} section after retry.")
                        if section_name == "basics":
                            logger.error("❌ Basics section is required. Aborting extraction.")
                            return None
                        else:
                            logger.warning(f"⚠️ Degrading gracefully, omitting section: {section_name}")
                except Exception as e:
                    logger.error(f"❌ Error extracting {section_name} section: {e}")
                    if section_name == "basics":
                        logger.error("❌ Basics section error is fatal. Aborting extraction.")
                        return None
                    else:
                        logger.warning(f"⚠️ Degrading gracefully, omitting section: {section_name} after error")
