...
from app.core.models import GitHubProfile
from app.services.pdf import logger
...
def _fetch_github_api(api_url, params=None, cache_ttl_seconds=None):
    headers = {}
    ...
    use_cache = DEVELOPMENT_MODE or cache_ttl_seconds is not None
...
def fetch_repo_contributors(owner: str, repo_name: str) -> list[dict]:
    try:
        ...
        # Cache contributors for 24 hours (86400 seconds)
        status_code, contributors_data = _fetch_github_api(api_url, cache_ttl_seconds=86400)
...
def _process_single_repo(repo: Dict, username: str) -> Optional[Dict]:
    if repo.get("fork") and repo.get("forks_count", 0) < 5:
        return None

    repo_name = repo.get("name")
    
    # Pre-filter for small or old repos
    size = repo.get("size", 0) # GitHub API returns size in KB
    ...
    if pushed_at_str:
        try:
            ...
            if (datetime.datetime.utcnow() - pushed_at).days > 730:
                is_old = True
        except ValueError:
            pass
    ...
    if is_old or is_trivial:
        # Default to self_project
        contributors_data = [{"login": username, "contributions": 1}]
    else:
    ...
    try:
        ...
        if status_code == 200:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                future_to_repo = {
                    executor.submit(_process_single_repo, repo, username): repo
